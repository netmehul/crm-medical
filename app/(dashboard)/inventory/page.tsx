...
import { Package, Plus, Minus, AlertTriangle, Pencil, Trash2, X } from "lucide-react";
...
type Category = "All" | "Medicines" | "Equipment" | "Consumables" | "Samples";
...
export default function InventoryPage() {
  ...
  const categories: Category[] = ["All", "Medicines", "Equipment", "Consumables", "Samples"];
  ...
  const handleStockTransaction = () => {
    ...
    const newStock = type === "in" ? item.currentStock + stockQty : Math.max(0, item.currentStock - stockQty);
    setItems((prev) => prev.map((i) =>
      i.id === item.id ? {
        ...i,
        currentStock: newStock,
        status: newStock === 0 ? "Out of Stock" : newStock <= i.threshold ? "Low Stock" : "In Stock",
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className="space-y-5">
        ...
        {stockModal && (
          <div className="space-y-4">
            ...
            <div className="flex gap-3">
              ...
              <Button variant={stockModal.type === "in" ? "success" : "danger"} onClick={handleStockTransaction} disabled={stockQty <= 0}>
      ...
      {/* Add Item Modal */}
      <Modal isOpen={addItemOpen} onClose={() => setAddItemOpen(false)} title="Add Inventory Item" size="md">
        <div className="space-y-4">
          <Input label="Item Name" placeholder="e.g. Amoxicillin 500mg" />
          <div className="grid grid-cols-2 gap-4">
            ...
            <Input label="Unit" placeholder="Tablets, Units, Pairs..." />
          </div>
          <div className="grid grid-cols-2 gap-4">
          ...
          <div className="flex gap-3 pt-2">
            ...
            <Button onClick={() => { addToast({ type: "success", title: "Item added to inventory" }); setAddItemOpen(false); }}>Add Item</Button>
          </div>
        </div>
      </Modal>
    </motion.div>
